...
if(PAGE===true) {
  ...
  if ($idseccion === 'elecciones' && $iddepto) {
    ...
    $cache_pattern .= "|$idseccion|portada";
  }
  ...
  $cache_pattern = "$cache_prefix|claves|" . preg_replace('(\.(.*))','',$_REQUEST['clave']);
...
if(!$smarty->is_cached('header2.tpl',$cache_pattern)) {
  ...
  if(!empty($row['menu'])) {
      ...
      if ( strtotime($Ed) <= strtotime("2011-08-22") ) {
          ...
          AND seccion.idseccion <> 5 AND seccion.idseccion <> 19");
          ...
          AND seccion.idseccion <> 10 AND seccion.idseccion <> 8 AND seccion.idseccion <> 9");
          $cambio = false;
...
if ($idseccion == 'elecciones' || in_array($idedicion,$electionsdays))
  $smarty->display('header_elecciones.tpl',$cache_pattern);
else 
  $smarty->display('header2.tpl',$cache_pattern);
